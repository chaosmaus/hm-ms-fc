$(document).ready(function () {
    ...
    const filterController = (data) => {
      ...
      $(".item").each((index, element) => {
        ...
        const matchedProperty = availableProperties.find( (item) => item.listingId === currentId )
        if (!matchedProperty){
          ...
          //console.log('matchedProperty: ', matchedProperty)
           showProperties.push(matchedProperty.listingId)
        }
        //console.log('index', index)
        /* 
        if (availableProperties.includes(currentId)) {
          if (!$(element).hasClass("hidden")) {
            $(element).addClass("hidden");
          ...
          showProperties.push(currentId);
        } */
  
        // second filter parameter -> numGuests should retrieve this information from the server, so its real time
       
        const matchedPropertyGuests = availableProperties.find( (item) => item.accommodates < guests )
        if (!matchedPropertyGuests){
          if (!$(element).hasClass("hidden")) {
            $(element).addClass("hidden");
          }
        }else{
          //console.log('matchedPropertyGuests: ', matchedPropertyGuests)
          if(!showProperties.find(matchedProperty.listingId))  showProperties.push(matchedPropertyGuests.listingId)
        }
      ...
      showProperties.forEach((listingId, index)=>{
        if($('.item').eq(index).find('.item_guesty-id').text() === listingId)  $('.item').eq(index).removeClass('hidden')
      })
